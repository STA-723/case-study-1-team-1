...
After grouping Gestational Age into binary response variable (Preterm Delivery or Non-preterm Delivery), in order to allow for non-linear dose-response relationship, we applied Generalized Additive Model (GAM) to fit the data.
...
Results from our Bayesian Generalized Additive Model align with previous results in frequentist approach. In Figure 4, higher concentration of DDE and PCBs is related to higher risks for pregnant women. In addition, both DDE and the first principle component have significant p-values, indicating the importance of these exposures in the relationship with the response. If we further consier use estimates of Bayesian GAM to check residual assumptions of GAM, Figure 5 verifies the correctness of Bayesian GAM, which outperforms that of frequentist approach. Estimated effects of DDE and PCBs also have narrower credible intervals compared to frequentist ones. Our model capture some flat regions when concentration is relatively low and high. 
...
library(cowplot)
library(ggplot2)
p1 <- ggdraw() + draw_image("GAMPlotMaternalAge.png", scale = 0.9)+
  ggtitle("Figure 4")
p2 <- ggdraw() + draw_image("GAMPlotCenter.png", scale = 0.9)+
  ggtitle("Figure 5")
pp1<- plot_grid(p1, p2)
ggdraw(add_sub(pp1, "Figure 4 & 5. Effect Plot for Maternal Age and Center", vpadding=grid::unit(0, "lines"), y = 12, x = 0.32, hjust = 0, size=8, fontface="bold"))
```
...
p1 <- ggdraw() + draw_image("BGAM.jpeg", scale = 0.9)+
  ggtitle("Figure 4")
ggdraw(add_sub(p1, "Figure 4. Nonlinear trend of several variables in Bayesian GAM", vpadding=grid::unit(0, "lines"), y = 3, x = 0.3, hjust = 0, size=8, fontface="bold"))
...
p1 <- ggdraw() + draw_image("BGAM2.jpeg", scale = 0.9)+
  ggtitle("Figure 4")
ggdraw(add_sub(p1, "Figure 5. Bayesian GAM model check", vpadding=grid::unit(0, "lines"), y = 3, x = 0.32, hjust = 0, size=8, fontface="bold"))
```
